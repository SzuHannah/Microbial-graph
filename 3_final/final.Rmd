...
```
## relate cluster to phyla composition  
```{r}
otus
...
## plot community composition by phyla
```{r}
##community composition by phyla
...
sample_ph<-selected.samples%>%select(ID,ph)%>%rename(SAMPLE=ID)
...
p7<-ggplot(sorpc_ph,aes(x = ph,y=log(abundance)))+
  geom_point()+
  facet_grid(PHYLUM~cluster,scales="free_y",labeller = labeller(cluster=new_labels))+
  theme_bw()+
  xlab("pH")+ylab("phyla composition")
p7
...
new_labels<-c("1"="Weathered (acidic)","2"="Cold moist","3"="Dry (basic)","4"="Short vegatation (neutral)","5"="Dry cold","6"="Disturbed")
#facet grid is clearer than facet wrap
p8<-ggplot(sorgc_ph,aes(x = ph,y=log(abundance)))+
  geom_point()+
  facet_grid(GENUS~cluster,scales="free_y",labeller = labeller(cluster=new_labels,GENUS=label_wrap_gen(5)))+
  theme_bw()+
